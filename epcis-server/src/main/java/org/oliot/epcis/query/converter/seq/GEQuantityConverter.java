...
/**
 * GE: Like EQ_quantity_uom, but includes events whose quantity-uom-pair (or a
 * corresponding quantity-uom-pair when an alternative uom is applied) is
 * greater than or equal to the specified parameter.
 * 
 */
public class GEQuantityConverter extends BaseConverter implements QueryConverter {
		...
		if (type.isEmpty()) {
			docList = List.of(
					new Document("quantityList",
							new Document("$elemMatch", new Document().append("quantity", new Document("$gte", d)))),
			...
			docList = List.of(
					new Document("quantityList",
							new Document("$elemMatch",
									new Document().append("uom", type).append("quantity", new Document("$gte", d)))),
					new Document("inputQuantityList",
							new Document("$elemMatch",
									new Document().append("uom", type).append("quantity", new Document("$gte", d)))),
