 ...
 * have an outputQuantityList field (that is, TransformationEvent or extension
 * event types that extend it); and where (b) one of the EPC classes listed in
 * the outputQuantityList field (depending on event type) matches one of the
 ...
 * 
 */
public class MATCHoutputEPCClassConverter extends BaseConverter implements QueryConverter {

	@Override
	public Document convert(String key, Object value) throws QueryParameterException {
		List<String> valueList = getListOfString(value);
		return getMATCHQuery("outputQuantityList.epcClass", valueList);
	}
}
