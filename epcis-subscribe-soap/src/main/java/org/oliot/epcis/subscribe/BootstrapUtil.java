...
import org.oliot.epcis.query.SOAPQueryServer;
...
import static org.oliot.epcis.subscribe.SOAPSubscribeServer.*;
...
public class BootstrapUtil {
	...
	private static void setConfigurationJson(String[] args) {
		...
		try {
			...
			try {
				...
				System.out.println("Oliot EPCIS X - Subscribe (SOAP) is running as developer mode (Eclipse)");
			} catch (Exception e1) {
				try {
					...
					System.out.println("Oliot EPCIS X - Subscribe (SOAP) is running as user mode (Runnable Jar)");
				} catch (Exception e2) {
	...
	private static void setLogger(String args[]) {
		try {
			...
			logger.info("Log4j appender configured with a file located in soapSubscribeConfiguration.json");
	...
	private static void setBackend(Vertx vertx) {
		try {
			...
			mSubscribeCollection = mDatabase.getCollection("Subscription", Document.class);
	...
	static void setHostPort() {
		...
		try {
			...
			logger.info("Oliot EPCIS X - Subscribe (SOAP) listens to the port " + SOAPQueryServer.port);
		} catch (Exception e) {
			port = 8082;
			logger.info("Oliot EPCIS X - Subscribe (SOAP) listens to the default port 8082");
		}
	}

	static void loadSubscriptions() {
		SubscriptionScheduler sub = new SubscriptionScheduler();
		sub.init();
	}
