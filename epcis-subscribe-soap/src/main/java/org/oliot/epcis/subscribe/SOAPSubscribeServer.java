 ...
 * <p>
 * Among various modules, epcis-subscribe-soap acts as a special server to
 * receive subscription queries to provide filtered, sorted, limited events in a
 * periodic manner or on demand.
 * <p>
 * 
 ...
 */
public class SOAPSubscribeServer extends AbstractVerticle {
	...
	public static MongoCollection<Document> mSubscribeCollection;
	...
	public static Logger logger = Logger.getLogger(SOAPSubscribeServer.class);
	...
	public void start(Promise<Void> startPromise) {
		...
		SOAPSubscribeService soapSubscribeService = new SOAPSubscribeService();
		...
		router.post("/epcis/query").consumes("*/xml").handler(routingContext -> {
			soapSubscribeService.run(routingContext.request(), routingContext.response().setChunked(true),
					routingContext.body().asString());
		});

		router.post("/epcis/test").handler(routingContext -> {
	...
	public static void main(String[] args) {
		...
		// vertx.deployVerticle(new SOAPSubscribeServer());
		
		DeploymentOptions dOptions = new DeploymentOptions().setInstances(numOfVerticles);
		vertx.deployVerticle("org.oliot.epcis.subscribe.SOAPSubscribeServer", dOptions);
	}
}
