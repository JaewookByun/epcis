...
public class XMLCaptureMetadataHandler {
    ...
    void registerBaseHandler(Router router) {
        ...
        router.options("/epcis").handler(routingContext -> {
            routingContext.response().putHeader("Allow", "OPTIONS").putHeader("GS1-EPCIS-Version", Metadata.GS1_EPCIS_Version)
                    .putHeader("GS1-Vendor-Version", Metadata.GS1_Vendor_Version).putHeader("GS1-CBV-Version", Metadata.GS1_CBV_Version)
        ...
        logger.info("[OPTIONS /epcis] - router added");
    }

    void registerCaptureHandler(Router router) {
        ...
        // the maximum payload size as count of EPCIS events (`GS1-EPCIS-Capture-Limit` header)
        // or as a maximum payload size in bytes (`GS1-EPCIS-Capture-File-Size-Limit` header)\n-
        ...
        router.options("/epcis/capture").handler(routingContext -> {
            routingContext.response().putHeader("Allow", "OPTIONS, GET, POST").putHeader("GS1-EPCIS-Version", Metadata.GS1_EPCIS_Version)
                    .putHeader("GS1-Vendor-Version", Metadata.GS1_Vendor_Version).putHeader("GS1-CBV-Version", Metadata.GS1_CBV_Version)
                    .putHeader("GS1-EPC-Format", GS1EPCFormat.Always_EPC_URN.toString())
                    .putHeader("GS1-CBV-XML-Format", GS1CBVXMLFormat.Always_URN.toString())
                    ...
                    .putHeader("GS1-EPCIS-Capture-File-Size-Limit", String.valueOf(Metadata.GS1_CAPTURE_file_size_limit))
                    .putHeader("GS1-EPCIS-Capture-Error-Behaviour", Metadata.GS1_EPCIS_Capture_Error_Behaviour)
                    .setStatusCode(204).end();
        });
        logger.info("[OPTIONS /epcis/capture] - router added");
    }

    void registerCaptureIDHandler(Router router) {
        ...
        router.options("/epcis/capture/:captureID").handler(routingContext -> {
            ...
            routingContext.response().putHeader("Allow", "OPTIONS, GET").putHeader("GS1-EPCIS-Version", Metadata.GS1_EPCIS_Version)
                    .putHeader("GS1-Vendor-Version", Metadata.GS1_Vendor_Version).putHeader("GS1-CBV-Version", Metadata.GS1_CBV_Version)
                    .putHeader("GS1-EPC-Format", GS1EPCFormat.Always_EPC_URN.toString())
                    .putHeader("GS1-CBV-XML-Format", GS1CBVXMLFormat.Always_URN.toString())
                    .putHeader("GS1-Extensions", Metadata.GS1_Extensions).setStatusCode(204).end();
        });
        logger.info("[OPTIONS /epcis/capture/:captureID] - router added");
    }

    void registerEventsHandler(Router router) {
        // Query metadata for the EPCIS events endpoint.
        // EPCIS 2.0 supports a number of custom headers to describe custom vocabularies and support multiple versions\n
        // of EPCIS and CBV. The `OPTIONS` method allows the client to discover which vocabularies and EPCIS and CBV\n
        // versions are used.\n"
        router.options("/epcis/events").handler(routingContext -> {
            routingContext.response().putHeader("Allow", "OPTIONS, POST").putHeader("GS1-EPCIS-Version", Metadata.GS1_EPCIS_Version)
                    .putHeader("GS1-Vendor-Version", Metadata.GS1_Vendor_Version).putHeader("GS1-CBV-Version", Metadata.GS1_CBV_Version)
                    .putHeader("GS1-EPC-Format", GS1EPCFormat.Always_EPC_URN.toString())
                    .putHeader("GS1-CBV-XML-Format", GS1CBVXMLFormat.Always_URN.toString())
                    .putHeader("GS1-Extensions", Metadata.GS1_Extensions).setStatusCode(204).end();
        });
        logger.info("[OPTIONS /epcis/events] - router added");
    }
}
