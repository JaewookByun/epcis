...
class JsonEventCaptureReadListener implements ReadListener {
	...
	JsonEventCaptureReadListener(ServletInputStream in, HttpServletResponse r, AsyncContext c) {
		input = in;
		res = r;
		ac = c;
	}
	...
	public void onAllDataRead() throws IOException {
		res.setStatus(200);
		ac.complete();
		JsonEventCapture jec = new JsonEventCapture();
		String inputString = queue.toString();
		if( inputString.length() >= 2 ){
			inputString = inputString.substring(1,inputString.length()-1);
		}
		jec.asyncPost(inputString);
	}
